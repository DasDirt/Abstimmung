...
public class ServerBootstrap extends ChannelInitializer<Channel> {
    ...
    public ServerBootstrap(int port) {
        try {
            ...
            serverBootstrap.group(eventLoopGroup = EPOLL ? new EpollEventLoopGroup() : new NioEventLoopGroup());
        ...
        if (channel != null) {
            if (channel.isOpen() && channel.isWritable()) {
                if (channel.eventLoop().inEventLoop()) {
                    ...
                    channel.eventLoop().execute(() -> channel.writeAndFlush(packet).addListeners(ChannelFutureListener.FIRE_EXCEPTION_ON_FAILURE));
                }
            }
        }
    }
    ...
    protected void initChannel(Channel channel) throws Exception {
        channel.pipeline().addLast("splitter", new PacketPrepender()).addLast("decoder", new PacketDecoder()).addLast("prepender", new PacketSplitter()).addLast("encoder", new PacketEncoder()).addLast(new PacketHandler());
    }
}
